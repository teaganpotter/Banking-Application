...
/**
 * Servlet implementation class SWithdrawServlet
 */
@WebServlet("/SWithdrawServlet")
public class SWithdrawServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SWithdrawServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (user.getHasOverdraftProtection()==true) {
			...
			if (user.getSavings().getBalance() + withdraw.getAmount() < 0 ) {
				System.out.println("Your transaction exceeds your balance. Your transaction could not be completed.");
			}else {
				user.getSavings().addTransaction(withdraw);
			}
		}
		else {
			Transaction withdraw = new Transaction("Withdraw", amount * -1, Calendar.getInstance().getTime());
				if(user.getSavings().getBalance() + withdraw.getAmount() < -500) {
					System.out.println("Your transaction exceeds -$500. Your transaction could not be completed.");
				} if ((user.getSavings().getBalance() + withdraw.getAmount()) >= -500 && (user.getSavings().getBalance() + withdraw.getAmount() < 0)){
